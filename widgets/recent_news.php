...
// Theme init
if (!function_exists('prohost_widget_recent_news_theme_setup')) {
	add_action( 'prohost_action_before_init_theme', 'prohost_widget_recent_news_theme_setup', 1 );
	function prohost_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('prohost_action_shortcodes_list',	'prohost_widget_recent_news_reg_shortcodes');
		if (function_exists('prohost_exists_visual_composer') && prohost_exists_visual_composer())
			add_action('prohost_action_shortcodes_list_vc','prohost_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('prohost_widget_recent_news_load')) {
	add_action( 'widgets_init', 'prohost_widget_recent_news_load' );
	function prohost_widget_recent_news_load() {
		register_widget('prohost_widget_recent_news');
	}
}


// Widget Class
class prohost_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'prohost'));
		parent::__construct( 'prohost_widget_recent_news', esc_html__('ProHost - Recent News', 'prohost'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = prohost_sc_recent_news( array(
			'from_widget'		=> true,
	...
	function form($instance) {
		...
		$news_styles = prohost_get_list_templates('news');
		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'prohost'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'prohost'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'prohost'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'prohost'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed:', 'prohost'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'prohost'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'prohost'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'prohost'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'prohost'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'prohost'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show counters:', 'prohost'); ?></label><br />
...
*/
if ( !function_exists( 'prohost_sc_recent_news' ) ) {
	function prohost_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (prohost_param_is_on($show_categories)) {
			if ( !prohost_storage_isset('categories_'.$category) ) {
				prohost_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = prohost_prepare_widgets_args(prohost_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (prohost_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (prohost_param_is_on($show_categories) && !prohost_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(prohost_param_is_on($show_categories) && !prohost_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = prohost_storage_get('categories_'.$category);
			if (prohost_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'prohost').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cat_list as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'prohost')
		...
		$args = prohost_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('prohost_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	prohost_require_shortcode("trx_recent_news", "prohost_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('prohost_widget_recent_news_reg_shortcodes_vc')) {
	//add_action('prohost_action_shortcodes_list_vc','prohost_widget_recent_news_reg_shortcodes_vc');
	function prohost_widget_recent_news_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Recent News", "prohost"),
				"description" => wp_kses_data( __("Insert recent news list", "prohost") ),
				...
				"params" => array(
					array(
						...
						"description" => wp_kses_data( __("Select style to display news list", "prohost") ),
						"class" => "",
						"admin_label" => true,
						"std" => 'news-magazine',
					...
					array(
						...
						"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", "prohost") ),
						"admin_label" => true,
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"heading" => esc_html__("Show categories", "prohost"),
						"description" => wp_kses_data( __("Show categories in the shortcode's header", "prohost") ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show counters", "prohost"),
						"description" => wp_kses_data( __("Show counters (comments and views) in the shortcode's footer", "prohost") ),
						'dependency' => array(
							'element' => 'style',
							'value' => 'news-magazine'
						),
					...
					array(
						...
						"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", "prohost") ),
						"group" => esc_html__('Query', 'prohost'),
						"class" => "",
					...
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", "prohost"),
					...
					array(
						"param_name" => "featured",
						"heading" => esc_html__("Featured posts", "prohost"),
						"description" => wp_kses_data( __("How many posts will be displayed as featured?", "prohost") ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'prohost'),
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"description" => wp_kses_data( __("How many columns use to show news list", "prohost") ),
						"group" => esc_html__('Query', 'prohost'),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'style',
					...
					array(
						...
						"group" => esc_html__('Query', 'prohost'),
						"class" => "",
						"value" => array_flip(prohost_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'prohost'),
						"class" => "",
						"value" => array_flip(prohost_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
						"heading" => esc_html__("List IDs", "prohost"),
						"description" => wp_kses_data( __("Comma separated list of post's ID. If set - parameters above (category, count, order, etc.) are ignored!", "prohost") ),
						"group" => esc_html__('Query', 'prohost'),
						"class" => "",
						"value" => "",
						"type" => "textfield"
