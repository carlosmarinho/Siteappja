...
// Theme init
if (!function_exists('prohost_widget_top10_theme_setup')) {
	add_action( 'prohost_action_before_init_theme', 'prohost_widget_top10_theme_setup', 1 );
	function prohost_widget_top10_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('prohost_action_shortcodes_list',	'prohost_widget_top10_reg_shortcodes');
		if (function_exists('prohost_exists_visual_composer') && prohost_exists_visual_composer())
			add_action('prohost_action_shortcodes_list_vc','prohost_widget_top10_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('prohost_widget_top10_load')) {
	add_action( 'widgets_init', 'prohost_widget_top10_load' );
	function prohost_widget_top10_load() {
		register_widget('prohost_widget_top10');
	}
}

// Widget Class
class prohost_widget_top10 extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_top10', 'description' => esc_html__('Top 10 posts by average reviews marks (by author and users)', 'prohost'));
		parent::__construct( 'prohost_widget_top10', esc_html__('ProHost - Top 10 Posts', 'prohost'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		$reviews_first_author = prohost_get_theme_option('reviews_first')=='author';
		$reviews_second_hide = prohost_get_theme_option('reviews_second')=='hide';
		...
		for ($i=0; $i<2; $i++) {
			...
			$post_rating = 'prohost_reviews_avg'.($i==0 ? '' : '2');
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_author')); ?>"><?php esc_html_e('Author rating tab title:', 'prohost'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_users')); ?>"><?php esc_html_e('Users rating tab title:', 'prohost'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_2"><?php esc_html_e('As stars', 'prohost'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('As icon', 'prohost'); ?></label>
...
*/
if ( !function_exists( 'prohost_sc_widget_top10' ) ) {
	function prohost_sc_widget_top10($atts, $content=null){	
		...
		extract($atts);
		$type = 'prohost_widget_top10';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_top10' 
								. (prohost_exists_visual_composer() ? ' vc_widget_top10 wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, prohost_prepare_widgets_args(prohost_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_top10', 'widget_top10') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('prohost_shortcode_output', $output, 'trx_widget_top10', $atts, $content);
	}
	prohost_require_shortcode("trx_widget_top10", "prohost_sc_widget_top10");
}


// Add [trx_widget_top10] in the VC shortcodes list
if (!function_exists('prohost_widget_top10_reg_shortcodes_vc')) {
	//add_action('prohost_action_shortcodes_list_vc','prohost_widget_top10_reg_shortcodes_vc');
	function prohost_widget_top10_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Top10 Reviews", "prohost"),
				"description" => wp_kses_data( __("Insert Top10 reviews list with thumbs and post's meta", "prohost") ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Author's tab title", "prohost"),
						"description" => wp_kses_data( __("Title for the tab with authors's reviews", "prohost") ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("User's tab title", "prohost"),
						"description" => wp_kses_data( __("Title for the tab with authors's reviews", "prohost") ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"value" => array_flip(array(
							'2' => esc_html__('As stars', 'prohost'),
							'1' => esc_html__('As text', 'prohost'),
							'0' => esc_html__('Hide', 'prohost')
						)),
						"type" => "dropdown"
					),
