...
if ( !function_exists( 'prohost_template_single_standard_theme_setup' ) ) {
	add_action( 'prohost_action_before_init_theme', 'prohost_template_single_standard_theme_setup', 1 );
	function prohost_template_single_standard_theme_setup() {
		prohost_add_template(array(
			'layout' => 'single-standard',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'prohost'),
			'thumb_title'  => esc_html__('Fullwidth image (crop)', 'prohost'),
			'w'		 => 1170,
			'h'		 => 659
		));
	}
...
// Template output
if ( !function_exists( 'prohost_template_single_standard_output' ) ) {
	function prohost_template_single_standard_output($post_options, $post_data) {
		...
		$title_tag = prohost_get_custom_option('show_page_title')=='yes' ? 'h3' : 'h1';
		...
		if ($post_data['post_type'] != "page" && $show_title && $post_options['location'] == 'center' && prohost_get_custom_option('show_page_title')=='no') {
			?>
		...
		if (!$post_data['post_protected'] && (
			...
			(prohost_get_custom_option('show_featured_image')=='yes' && $post_data['post_thumb'])	// && $post_data['post_format']!='gallery' && $post_data['post_format']!='image')
		)) {
			?>
		...
		if (!$post_data['post_protected'] && prohost_get_custom_option('show_post_info')=='yes') {
			$post_options['info_parts'] = array('snippets'=>false, 'terms'=>false);
		...
		if ($post_data['post_protected']) { 
			...
			if ( prohost_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				...
				<div class="post_info post_info_bottom">
					<span class="post_info_item post_info_tags"><?php esc_html_e('Tags:', 'prohost'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php 
			}
		} 
		...
		if (!$sidebar_present) prohost_close_wrapper();	// .post_item
