...
if ( !function_exists( 'prohost_template_header_2_theme_setup' ) ) {
	add_action( 'prohost_action_before_init_theme', 'prohost_template_header_2_theme_setup', 1 );
	function prohost_template_header_2_theme_setup() {
		prohost_add_template(array(
			'layout' => 'header_2',
			'mode'   => 'header',
			'title'  => esc_html__('Header 2', 'prohost'),
			'icon'   => prohost_get_file_url('templates/headers/images/2.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'prohost_template_header_2_output' ) ) {
	function prohost_template_header_2_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_2 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_2 top_panel_position_<?php echo esc_attr(prohost_get_custom_option('top_panel_position')); ?>">
				...
				<div class="content_wrap">
					<div class="columns_wrap columns_fluid"><?php
						...
						if (function_exists('prohost_exists_woocommerce') && prohost_exists_woocommerce() && (prohost_is_woocommerce_page() && prohost_get_custom_option('show_cart')=='shop' || prohost_get_custom_option('show_cart')=='always') && !(is_checkout() || is_cart() || defined('WOOCOMMERCE_CHECKOUT') || defined('WOOCOMMERCE_CART'))) {
							?><div class="column-1_4 contact_field contact_cart"><?php get_template_part(prohost_get_file_slug('templates/headers/_parts/contact-info-cart.php')); ?></div><?php
						}
						?></div>
				</div>
			</div>
