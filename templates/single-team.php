...
if ( !function_exists( 'prohost_template_single_team_theme_setup' ) ) {
	add_action( 'prohost_action_before_init_theme', 'prohost_template_single_team_theme_setup', 1 );
	function prohost_template_single_team_theme_setup() {
		prohost_add_template(array(
			'layout' => 'single-team',
			'mode'   => 'team',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Team member', 'prohost'),
			'thumb_title'  => esc_html__('Large image (crop)', 'prohost'),
			'w'		 => 770,
			'h'		 => 434
		));
	}
...
// Template output
if ( !function_exists( 'prohost_template_single_team_output' ) ) {
	function prohost_template_single_team_output($post_options, $post_data) {
		$post_data['post_views']++;
		$show_title = prohost_get_custom_option('show_post_title')=='yes';
		...
		if ($show_title && $post_options['location'] == 'center' && prohost_get_custom_option('show_page_title')=='no') {
			?>
		...
		prohost_open_wrapper('<section class="post_content'.(!$post_data['post_protected'] && $post_data['post_edit_enable'] ? ' '.esc_attr('post_content_editor_present') : '').'" itemprop="articleBody">');
