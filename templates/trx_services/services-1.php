...
if ( !function_exists( 'prohost_template_services_1_theme_setup' ) ) {
	add_action( 'prohost_action_before_init_theme', 'prohost_template_services_1_theme_setup', 1 );
	function prohost_template_services_1_theme_setup() {
		prohost_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'prohost'),
...
// Template output
if ( !function_exists( 'prohost_template_services_1_output' ) ) {
	function prohost_template_services_1_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = prohost_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
				...
				<div class="sc_services_item_content">
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !prohost_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore sc_button sc_button_square sc_button_style_filled sc_button_size_small"><?php echo trim($post_options['readmore']); ?></a><?php
							}
						}
						?>
					</div>
