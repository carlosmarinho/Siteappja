...
if ( !function_exists( 'prohost_template_team_2_theme_setup' ) ) {
	add_action( 'prohost_action_before_init_theme', 'prohost_template_team_2_theme_setup', 1 );
	function prohost_template_team_2_theme_setup() {
		prohost_add_template(array(
			'layout' => 'team-2',
			'template' => 'team-2',
			'mode'   => 'team',
			/*'container_classes' => 'sc_slider_pagination sc_slider_pagination_bottom sc_slider_nocontrols',*/
			'title'  => esc_html__('Team /Style 2/', 'prohost'),
...
// Template output
if ( !function_exists( 'prohost_template_team_2_output' ) ) {
	function prohost_template_team_2_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			...
			><div class="sc_team_item_avatar column-2_5">
				...
				<div class="sc_team_item_description"><?php echo trim(prohost_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : prohost_get_custom_option('post_excerpt_maxlength_masonry'))); ?></div>
