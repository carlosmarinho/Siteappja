...
if ( !function_exists( 'prohost_template_list_theme_setup' ) ) {
	add_action( 'prohost_action_before_init_theme', 'prohost_template_list_theme_setup', 1 );
	function prohost_template_list_theme_setup() {
		prohost_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'prohost')
			));
	}
}

// Template output
if ( !function_exists( 'prohost_template_list_output' ) ) {
	function prohost_template_list_output($post_options, $post_data) {
