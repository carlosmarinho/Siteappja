...
function prohost_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = PROHOST_STORAGE['co_add_params']!=undefined ? PROHOST_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#prohost-customizer-styles-'+option).length > 0 ? jQuery('#prohost-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="prohost-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
